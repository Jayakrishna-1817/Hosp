...
const app = express();
const dburl = process.env.DB_URL || "mongodb+srv://jk1817:test123@vijay.t7vymtr.mongodb.net/vvit?retryWrites=true&w=majority&appName=vijay";
...
app.get("/details", async (req, res) => {
    try {
      ...
      res.render("details", { hospitals }); // Pass the records to the template
    } catch (error) {
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            ...
            const hospital = new Hospital({ email, password: hashedPassword, username: uname  });
            await hospital.save();
